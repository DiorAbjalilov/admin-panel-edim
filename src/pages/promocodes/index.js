...
const Promocodes = () => {
  ...
  const columns = useMemo(() => {
    let cols = [
      {
        ...
        dataIndex: "code",
        key: "code",
        render: (value, obj) => <span>{value}</span>,
      },
      {
        title: "Скидка",
        render: (_, obj) => <span>{obj.discount} so'm.</span>,
      },
      {
        title: "Опубликован",
        render: (_, obj) => obj.publish && <CheckIcon />,
      },
    ];
    ...
    if (admin && !moderator) {
      cols.push({
        title: "Ресторан",
        render: (_, obj) => <span>{obj?.restaurant?.name}</span>,
      });
    }
      ...
      render: (_, obj) => (
        <>
          <Popconfirm
            ...
            onConfirm={() => {
              deletePromocode({ variables: { where: { id: obj.id } } });
            }}
            okText="Да"
            cancelText="Нет"
  ...
  const promocodes = useMemo(
    () => (data ? data.findManyPromocode : []),
    [data],
  );
  const promocodesCount = useMemo(
    () => (data ? data.findManyPromocodeCount : []),
    [countData],
  );

  const [deletePromocode, { loading: deleteLoading }] = useMutation(
    DELETE_ONE_PROMOCODE,
    {
      onCompleted: () => {
      ...
      update: (client, { data: { deleteOnePromocode } }) => {
        ...
        if (prevData) {
          ...
          client.writeQuery({
            ...
            variables,
            data: {
              findManyPromocode: findManyPromocode.filter(
                (item) => item.id !== deleteOnePromocode.id,
              ),
            },
          });
        }
  ...
  return (
    <>
      ...
      >
        <Form.Item name="search" style={{ marginRight: 15 }}>
          <Input
            name="search"
            placeholder="Найти"
            allowClear
