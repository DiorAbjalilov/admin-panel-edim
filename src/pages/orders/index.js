...
const Orders = () => {
  ...
  const { data, loading } = useQuery(FIND_MANY_ORDER, {
    ...
    onError: (e) => {
      message.error(
        "Не удалось получить информацию о заказзах, повторите попытку позже",
      );
    },
  ...
  const restaurants = useMemo(
    () =>
      restaurantData && restaurantData.findManyRestaurant
        ? restaurantData.findManyRestaurant
        : [],
    [restaurantData],
  );
  ...
  return (
    <>
      ...
      <Form name="order-filter" layout="horizontal" onFinish={onFormSubmit}>
        ...
        <Form.Item name={"restaurant"} className="item">
          <Select
        ...
        columns={[
          {
            ...
            render: (restaurant, obj) => (
              <Link to={`/${type}/restaurant/${restaurant.id}`}>
                {restaurant.name}
              </Link>
            ),
          },
