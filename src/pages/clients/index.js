...
const Clients = () => {
  ...
  const [reorderModerators, { loading: reorderLoading }] =
    useMutation(REORDER_MODERATORS);
  ...
  const moderatorsTotal = useMemo(
    () => (data ? data.findManyModeratorCount : 0),
    [data],
  );
  ...
  const DraggableBodyRow = useCallback(
    ({ className, style, ...restProps }) => {
      ...
      const index = moderators.findIndex(
        (x) => x.weight === restProps["data-row-key"],
      );
  ...
  return (
    <>
      ...
      />
      <Form name="clients-filter" layout="horizontal" onFinish={onFormSubmit}>
        ...
        <Form.Item name={"status"} className="item">
          ...
          >
            ...
            <Select.Option value={true}>Удалённый</Select.Option>
          </Select>
        </Form.Item>
        <Form.Item>
        ...
        columns={[
          ...
          },
          {
            ...
            render: (_, object) => (
              ...
              >
                <Button type={object.delete ? "primary" : "danger"}>
                  {object.delete ? "Восстановить" : "Удалить"}
                </Button>
              </Popconfirm>
            ),
